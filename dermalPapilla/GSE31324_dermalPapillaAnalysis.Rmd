...
GSE31324Data= distinct(GSE31324Data, Ensembl, .keep_all = T)
#GSE31324Data= GSE31324SeriesMatrix
write.csv(GSE31324Data, "GSE31324Data.txt", row.names = F)
GSE31324Data <-read_csv("GSE31324Data.txt")

variance <- apply(GSE31324Data[3:14],1,sd)/apply(GSE31324Data[3:14],1,mean)
hist(variance)
keep=as.data.frame(GSE31324Data)
rownames(keep)=keep[,1]
keep=keep[3:14]
keep2=which(apply(keep,1,sd)/apply(keep,1,mean)>= 0.3)
keep=keep[keep2,]
hist(keep)
keep3= mutate(keep, ensembl=rownames(keep))
GSE31324Data=keep3
```
...
datExpr31324 = t(data31324[,1:12]);
colnames(datExpr31324) = data31324$ensembl
...
# Start the plot
sizeGrWindow(10, 5);
pdf(file ="NeuronsvsGSE31324Data-Zsummary-medianRank.pdf", wi=10, h=5)
par(mfrow = c(1,2))
